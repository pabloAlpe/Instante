...
function updateParameters() {
  ...
  for (let i = 0; i < colors.length; i++) {
    if (m4 >= (1 / colors.length) * i) {
      brushColor = colors[i];
      paintScale = map(m4, (1 / colors.length) * i, (1 / colors.length) * (i + 1), paintScaleRanges.min, paintScaleRanges.max);
    } else {
      break;
    }
  }

  potencia = map(m3,0,1,0.,.2);

  mPosX = map(randomM4(m4),0.,1.,(-canvas.width/2)*.9,(canvas.width/2)*.9);
  mPosY=map(randomM4(m4),0.,1.,(-canvas.height/2)*.9,(canvas.height/2)*.9);
...
function drawScreen() {
  ...
  shader1.setUniform('u_potencia', potencia);
  shader1.setUniform('u_col', [246/255., 241/255., 233/255.]);
